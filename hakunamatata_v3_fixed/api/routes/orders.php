...
// Produces D0001–D9999 for online/delivery orders (channel=web)
// Produces A0001–A9999 for cashier/counter orders
// Counter resets every day. Stored in daily_order_counter table.
//
...
//   ) ENGINE=InnoDB;
//
function generateDailyOrderRef(string $prefix = 'D'): string {
    $prefix = strtoupper(substr($prefix, 0, 1));
    $today  = date('Y-m-d');
    $pdo    = db();

    // Atomic upsert — LAST_INSERT_ID() trick returns the incremented value
    // in one round-trip, eliminating the race condition that caused duplicate
    // order_ref entries when two orders were placed at the same moment.
    $pdo->prepare(
        "INSERT INTO daily_order_counter (counter_date, prefix, last_seq)
         VALUES (?, ?, 1)
         ON DUPLICATE KEY UPDATE last_seq = LAST_INSERT_ID(last_seq + 1)"
    )->execute([$today, $prefix]);

    $seq = (int)$pdo->lastInsertId();

    // Safety bounds
    if ($seq < 1)    $seq = 1;
    if ($seq > 9999) $seq = ($seq % 9999) ?: 9999;
...
function fetchOrder(int $orderId): ?array {
    ...
    // Only expose the plaintext OTP while out for delivery or driver arrived, and not yet verified
    if (!in_array($order['order_status'], ['out_for_delivery', 'arrived']) || $order['otp_verified']) {
        unset($order['delivery_otp']);
    }
    return $order;
}
...
if ($m === 'POST' && !$id && $action !== 'checkout') {
    ...
    try {
        ...
        // Cashier orders: A0001-A9999; online channel gets D-prefix
        $prefix = ($b['channel'] ?? '') === 'web' ? 'D' : 'A';
        $ref    = generateDailyOrderRef($prefix);
        ...
        // Delivery record (for delivery orders only — not pickup/collection)
        if ($b['order_type'] === 'delivery') {
            $db->prepare(
                "INSERT INTO delivery (order_id, delivery_status) VALUES (?, 'pending')"
            )->execute([$orderId]);
            // Driver will be assigned when kitchen marks order ready
        }
        // Pickup/collection: no delivery record, no driver assigned (A-series)
        ...
        pushNotification($auth['user_id'], 'order_update',
            '✅ Order Placed',
            "Your order $ref has been placed and is awaiting kitchen confirmation.",
            ['order_id' => $orderId]
        );
        ...
        respondError($e->getMessage() ?: 'Order failed.', 422);
    }
}
...
if ($m === 'PATCH' && $id && $action === 'status') {
    ...
    $order = db()->prepare(
        "SELECT order_id, order_status, user_id, order_type, order_ref FROM sale_order WHERE order_id=? LIMIT 1"
    );
    $order->execute([$id]);
    $order = $order->fetch();
    if (!$order) respondError('Order not found', 404);
    ...
    // Notify customer
    $messages = [
        'preparing'        => ['✅ Order Ready', 'Your order is ready and a driver is being assigned!'],
        'out_for_delivery' => ['🚗 Order On Its Way', 'Your driver is heading to you now!'],
        'delivered'        => ['📦 Order Delivered', 'Your order has been delivered. Enjoy!'],
        'cancelled'        => ['❌ Order Cancelled', 'Your order has been cancelled.'],
    ];
    if (isset($messages[$new]))
        pushNotification($order['user_id'], 'order_update',
            $messages[$new][0], $messages[$new][1], ['order_id' => $id]);

    // ── AUTO-ASSIGN DRIVER when kitchen marks order ready (preparing) ──────
    if ($new === 'preparing' && $order['order_type'] === 'delivery') {
        try {
            // Ensure a delivery row exists (may be missing if order came through cashier flow)
            $delivCheck = db()->prepare("SELECT delivery_id FROM delivery WHERE order_id=? LIMIT 1");
            $delivCheck->execute([$id]);
            if (!$delivCheck->fetch()) {
                db()->prepare("INSERT INTO delivery (order_id, delivery_status) VALUES (?, 'pending')")
                   ->execute([$id]);
            }
            ...
            if ($driver) {
                ...
                db()->prepare(
                    "INSERT INTO order_status_log (order_id, changed_by, from_status, to_status, notes)
                     VALUES (?,?,?,?,'Driver auto-assigned')"
                )->execute([$id, $auth['user_id'], $new, $new]);

                $orderRef = $order['order_ref'] ?? "#$id";
                pushNotification($driverId, 'delivery',
                    '📦 New Delivery Assigned',
                    "Order $orderRef is ready for pickup. Head to the restaurant.",
                    ['order_id' => $id]
                );
            }
        } catch (Throwable $e) {
            respondError('Driver assign: '.$e->getMessage(), 500);
        }
    }

    // ── GENERATE OTP when driver clicks "I'm On The Way" (out_for_delivery) ─
    // 4-digit OTP generated here, saved plaintext + hash, sent to customer.
    // Delivery status → out_for_delivery (driver is now "assigned/busy").
    if ($new === 'out_for_delivery') {
        try {
            $existing = db()->prepare(
                "SELECT delivery_otp_hash FROM delivery WHERE order_id=? LIMIT 1"
            );
            $existing->execute([$id]);
            ...
            if ($existing && empty($existing['delivery_otp_hash'])) {
                db()->prepare(
                ...
                // Re-generate OTP even if one existed (driver may have tapped again)
                db()->prepare(
                    "UPDATE delivery SET delivery_otp_hash=?, delivery_otp=?,
                     delivery_status='out_for_delivery',
                     dispatched_at=COALESCE(dispatched_at, NOW()) WHERE order_id=?"
            ...
            // OTP failure must not block the status update
        }
    }
...
if ($m === 'PATCH' && $id && $action === 'cancel') {
    ...
    if (!in_array($order['order_status'], ['pending','preparing']))
        respondError('Order cannot be cancelled — it is already out for delivery.', 400);
    ...
    db()->prepare(
        "INSERT INTO order_status_log (order_id, changed_by, from_status, to_status, notes)
         VALUES (?,?,?,?,'Customer cancelled')"
    )->execute([$id, $auth['user_id'], $order['order_status'], 'cancelled']);
...
if ($m === 'POST' && !$id && $action === 'checkout') {
    ...
    try {
        ...
        // 2. Create sale_order
        $ref = generateDailyOrderRef('D'); // Online orders: D0001-D9999
        $db->prepare(
            "INSERT INTO sale_order
             ...
             VALUES (?,?,'delivery','web',?,?,?,?,'Standard',?,?,'pending')"
        )->execute([
            $ref, $auth['user_id'],
        ...
        // 5. Delivery record — driver will be assigned when kitchen marks order ready
        $db->prepare(
            "INSERT INTO delivery (order_id,delivery_status) VALUES (?,'pending')"
        )->execute([$orderId]);
        ...
        $db->prepare(
            "INSERT INTO order_status_log
             ...
             VALUES (?,?,NULL,'pending','Web checkout — payment processed')"
        )->execute([$orderId, $auth['user_id']]);
        ...
        pushNotification(
            ...
            '✅ Order Placed',
            "Your order $ref has been placed! The kitchen will confirm it shortly.",
            ['order_id' => $orderId]
        );
        ...
        respondError($outerErr->getMessage() ?: 'Checkout failed.', 422);
    }
}
